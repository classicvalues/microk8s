...
def retry_run(*args, die=True, debug=False, stdout=True, times=3):
    for attempt in range(1, times + 1):
        try:
            return run(
                *args, die=(times == attempt and die), debug=debug, stdout=stdout
            )
...
def enable_addons(
    debug, addons=("dns", "storage", "ingress", "metallb:10.64.140.43-10.64.140.49")
):
    for addon in addons:
...
def parse_hostname(hostname: str) -> ParseResult:
    ...
    if parsed.path not in ("", "/"):
        print(
            "WARNING: The path `%s` was set on the hostname, but was ignored."
            % parsed.path
        )

    if parsed.params:
        print(
            "WARNING: The params `%s` were set on the hostname, but were ignored."
            % parsed.params
        )

    if parsed.query:
        print(
            "WARNING: The query `%s` was set on the hostname, but was ignored."
            % parsed.query
        )

    if parsed.params:
        print(
            "WARNING: The fragment `%s` was set on the hostname, but was ignored."
            % parsed.fragment
        )
...
def print_info(hostname, password):
    ...
    if charm_exists("istio-ingressgateway"):
        ...
        print(
            "For more information on how to use Kubeflow, see https://www.kubeflow.org/docs/"
        )
